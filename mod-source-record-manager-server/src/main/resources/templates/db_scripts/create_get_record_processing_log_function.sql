...
BEGIN
    RETURN QUERY
        WITH temp_result AS (
        SELECT id, journal_records.job_execution_id, journal_records.source_id,
            journal_records.entity_type, journal_records.entity_id, journal_records.entity_hrid,
                 CASE WHEN action_type = 'PARSE'
                    ...
                    WHEN action_type = 'UPDATE'
                        THEN 'UPDATED'
                 END
                 AS action_type, journal_records.action_status, journal_records.action_date, journal_records.source_record_order, journal_records.error, action_type_by_source.title, journal_records.tenant_id, journal_records.instance_id, journal_records.holdings_id, journal_records.order_id, journal_records.permanent_location_id
    	...
    	INNER JOIN
    		(
    		    SELECT entity_type as entity_type_max,
    		        ...
    		        MAX(title) AS title  -- Select the non-NULL title when available
            	FROM journal_records
