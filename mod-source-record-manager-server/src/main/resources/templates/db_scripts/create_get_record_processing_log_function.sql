...
BEGIN
    RETURN QUERY
        WITH temp_result AS (
            SELECT journal_records.id,
                   journal_records.job_execution_id,
                   journal_records.source_id,
                   journal_records.entity_type,
                   journal_records.entity_id,
                   ...
                   journal_records.action_date,
                   journal_records.source_record_order,
                   journal_records.error,
                   action_type_by_source.title AS title,
                   journal_records.tenant_id,
                   journal_records.instance_id,
                   journal_records.holdings_id,
                   journal_records.order_id,
                   journal_records.permanent_location_id
            ...
            SELECT journal_records.id,
                   ...
                   journal_records.entity_id,
                   journal_records.entity_hrid,
                   CASE WHEN action_type_by_source.error_max != '' OR journal_records.action_type = 'MATCH' THEN 'DISCARDED' END AS action_type,
                   journal_records.action_status,
                   journal_records.action_date,
                   journal_records.source_record_order,
                   journal_records.error,
            ...
            INNER JOIN (
                SELECT entity_type as entity_type_max,
                       ...
                       action_status as action_status_max,
                       MAX(error) AS error_max,
                       (array_agg(id ORDER BY array_position(array['NON_MATCH', 'MATCH'], action_type)))[1] AS id_max,
                       MAX(journal_records.title) AS title
                FROM journal_records
                WHERE journal_records.job_execution_id = jobExecutionId
                  AND journal_records.source_id = recordId
                  ...
                  AND action_type = 'MATCH'
                  AND NOT EXISTS (
                      SELECT 1 FROM journal_records
                      WHERE journal_records.job_execution_id = jobExecutionId
                        AND journal_records.source_id = recordId
                        AND action_type NOT IN ('MATCH', 'PARSE')
                  )
                GROUP BY entity_type, entity_id, action_status
            ) AS action_type_by_source
            ON journal_records.id = action_type_by_source.id_max
        ...
        (
            SELECT
              ...
              COALESCE(marc.title,instances.title,holdings.title,items.title,po_lines.title) AS title,
            ...
            LEFT JOIN
              ...
              WHERE entity_type IN ('MARC_BIBLIOGRAPHIC', 'MARC_HOLDINGS', 'MARC_AUTHORITY', 'PO_LINE') AND entity_id IS NULL AND NOT EXISTS
                  (SELECT 1
                   ...
                   WHERE tr2.entity_type IN ('MARC_BIBLIOGRAPHIC', 'MARC_HOLDINGS', 'MARC_AUTHORITY', 'PO_LINE') AND tr2.source_id = temp_result.source_id and tr2.entity_id IS NOT NULL
                   )
              ) AS marc
                ON marc.source_id = parsed.source_id
            LEFT JOIN
                (
                SELECT action_type, entity_id, temp_result.source_id, entity_hrid, error, temp_result.job_execution_id, temp_result.title as title, temp_result.source_record_order, temp_result.tenant_id
                FROM temp_result WHERE entity_type = 'INSTANCE' AND entity_id IS NOT NULL
                UNION ALL
                SELECT action_type, entity_id, temp_result.source_id, entity_hrid, error, temp_result.job_execution_id, temp_result.title as title, temp_result.source_record_order, temp_result.tenant_id
                FROM temp_result WHERE entity_type = 'INSTANCE' AND entity_id IS NULL AND NOT EXISTS
                    (
                        SELECT 1
                        FROM temp_result as tr2
            ...
            LEFT JOIN
                (
                SELECT action_type, entity_id, temp_result.source_id, error, temp_result.job_execution_id, temp_result.title as title, temp_result.source_record_order
                FROM temp_result WHERE entity_type = 'AUTHORITY'
                ) AS authority
                ON authority.source_id = marc.source_id
            LEFT JOIN
                (
                    SELECT action_type, entity_id, entity_hrid, temp_result.source_id, error,order_id, temp_result.job_execution_id, temp_result.title as title, temp_result.source_record_order
            ...
            FULL JOIN
                (SELECT tmp.action_type, tmp.entity_type, tmp.entity_id, tmp.entity_hrid, tmp.error, tmp.instance_id,
                        ...
                        FROM temp_result nested_result) AS joining_table
                         ...
                         WHERE  tmp.entity_type='HOLDINGS') AS holdings
            ...
            LEFT JOIN (
              ...
              FROM temp_result WHERE entity_type IN ('MARC_BIBLIOGRAPHIC', 'MARC_HOLDINGS', 'MARC_AUTHORITY') AND entity_id IS NOT NULL
                ...
                ON marc.source_id = marc_identifiers.marc_source_id
