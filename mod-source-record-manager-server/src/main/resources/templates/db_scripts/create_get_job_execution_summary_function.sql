...
BEGIN
  RETURN QUERY
    SELECT
      ...
      COUNT(DISTINCT(source_id)) FILTER (WHERE entity_type IN ('MARC_BIBLIOGRAPHIC', 'MARC_HOLDINGS', 'MARC_AUTHORITY') AND action_type in ('CREATE','MODIFY') AND action_status = 'COMPLETED') AS total_created_source_records,
