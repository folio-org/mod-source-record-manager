...
RETURNS TABLE(
  ...
  total_created_po_lines bigint, total_updated_po_lines bigint, total_discarded_po_lines bigint, total_po_lines_errors bigint,
...
BEGIN
  RETURN QUERY
    SELECT
      ...
      COUNT(*) FILTER (WHERE entity_type = 'PO_LINE' AND action_type = 'CREATE' AND action_status = 'COMPLETED') AS total_created_po_lines,
      COUNT(*) FILTER (WHERE entity_type = 'PO_LINE' AND action_type = 'UPDATE' AND action_status = 'COMPLETED') AS total_updated_po_lines,
      COUNT(*) FILTER (WHERE entity_type = 'PO_LINE' AND (action_type = 'NON_MATCH' OR action_status = 'ERROR')) AS total_discarded_po_lines,
      COUNT(*) FILTER (WHERE entity_type = 'PO_LINE' AND action_status = 'ERROR') AS total_po_lines_errors,
