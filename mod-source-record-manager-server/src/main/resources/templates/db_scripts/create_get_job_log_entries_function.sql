...
BEGIN
    RETURN QUERY EXECUTE format('
        ...
        FROM (
               ...
               WHERE journal_records.job_execution_id = ''%s''
               GROUP BY journal_records.source_id, journal_records.source_record_order, journal_records.job_execution_id
             ) AS records_actions
        LEFT JOIN (
                    SELECT journal_records.source_id, journal_records.error FROM journal_records
                    WHERE journal_records.job_execution_id = ''%s''
        ...
        ORDER BY %I %s
        LIMIT %s OFFSET %s;',
        jobExecutionId, jobExecutionId, jobExecutionId, sortingField, sortingDir, limitVal, offsetVal);
END;
$$ LANGUAGE plpgsql;
