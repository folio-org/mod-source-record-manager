...
v_sortingField text DEFAULT sortingfield;      -- for bottomSQL | originalSQL
  ...
  topSQL TEXT := '
    ...
    total_count_cte AS (
      SELECT count(*) AS total_count FROM qualifying_source_ids
    ),

    paginated_source_ids AS (
      SELECT
        source_id
      FROM qualifying_source_ids
      ORDER BY
        CASE
          WHEN ''%5$s'' = ''source_record_order'' THEN to_char((SELECT MIN(source_record_order) FROM journal_records WHERE source_id = qualifying_source_ids.source_id AND job_execution_id = ''%1$s''), ''FM0000000000'')
          WHEN ''%5$s'' = ''sort_title'' THEN (SELECT MIN(title) FROM journal_records WHERE source_id = qualifying_source_ids.source_id AND job_execution_id = ''%1$s'')
          WHEN ''%5$s'' = ''sort_error'' THEN (SELECT MIN(error) FROM journal_records WHERE source_id = qualifying_source_ids.source_id AND job_execution_id = ''%1$s'' AND error != '''')
    ...
    relevant_records AS (
      SELECT *
      FROM journal_records
      WHERE job_execution_id = ''%1$s'' AND source_id IN (SELECT source_id FROM paginated_source_ids)
    ),
    ...
    instances AS (
      ...
      WHERE entity_type = ''INSTANCE'' AND entity_id IS NULL AND NOT EXISTS
          (SELECT 1 FROM temp_result as tr2 WHERE tr2.entity_type = ''INSTANCE'' AND tr2.source_id = temp_result.source_id and tr2.entity_id IS NOT NULL)
    ),
    holdings AS (
        SELECT action_type, entity_type, entity_id, entity_hrid, error, instance_id,
            ...
            FROM temp_result WHERE entity_type = ''HOLDINGS'' AND entity_id IS NOT NULL
            UNION ALL
            SELECT action_type, entity_type, entity_id, entity_hrid, error, instance_id,
                 permanent_location_id, job_execution_id, source_id, title, source_record_order
            FROM temp_result WHERE entity_type = ''HOLDINGS'' AND entity_id IS NULL AND NOT EXISTS
                (SELECT 1 FROM temp_result as tr2 WHERE tr2.entity_type = ''HOLDINGS'' AND tr2.source_id = temp_result.source_id and tr2.entity_id IS NOT NULL)
    ),
    items AS (
      SELECT action_type, entity_id, holdings_id, entity_hrid, error, instance_id, job_execution_id, source_id, title, source_record_order
      FROM temp_result WHERE entity_type = ''ITEM'' AND entity_id IS NOT NULL
      UNION ALL
      SELECT action_type, entity_id, holdings_id, entity_hrid, error, instance_id, job_execution_id, source_id, title, source_record_order
      FROM temp_result WHERE entity_type = ''ITEM'' AND entity_id IS NULL AND NOT EXISTS
          (SELECT 1 FROM temp_result as tr2 WHERE tr2.entity_type = ''ITEM'' AND tr2.source_id = temp_result.source_id and tr2.entity_id IS NOT NULL)
    ),
    po_lines AS (
      SELECT action_type,entity_id,entity_hrid,source_id,error,order_id,job_execution_id,title,source_record_order
    ...
    authorities AS (
      ...
      UNION ALL
      SELECT action_type, entity_id, source_id, error, job_execution_id, title, source_record_order
      FROM temp_result WHERE entity_type = ''AUTHORITY'' AND entity_id IS NULL AND NOT EXISTS
          (SELECT 1 FROM temp_result as tr2 WHERE tr2.entity_type = ''AUTHORITY'' AND tr2.source_id = temp_result.source_id and tr2.entity_id IS NOT NULL)
    ),
    marc_authority AS (
      SELECT job_execution_id, entity_id, title, source_record_order, action_type, error, source_id, tenant_id
      FROM temp_result WHERE entity_type = ''MARC_AUTHORITY'' AND entity_id IS NOT NULL
      UNION ALL
      SELECT job_execution_id, entity_id, title, source_record_order, action_type, error, source_id, tenant_id
      FROM temp_result WHERE entity_type = ''MARC_AUTHORITY'' AND entity_id IS NULL AND NOT EXISTS
          (SELECT 1 FROM temp_result as tr2 WHERE tr2.entity_type = ''MARC_AUTHORITY'' AND tr2.source_id = temp_result.source_id and tr2.entity_id IS NOT NULL)
    ),
    marc_holdings AS (
      SELECT job_execution_id, entity_id, title, source_record_order, action_type, error, source_id, tenant_id
    ...
    marc_bibliographic AS (
      ...
      UNION ALL
      SELECT job_execution_id, entity_id, title, source_record_order, action_type, error, source_id, tenant_id
      FROM temp_result WHERE entity_type = ''MARC_BIBLIOGRAPHIC'' AND entity_id IS NULL AND NOT EXISTS
          (SELECT 1 FROM temp_result as tr2 WHERE tr2.entity_type = ''MARC_BIBLIOGRAPHIC'' AND tr2.source_id = temp_result.source_id and tr2.entity_id IS NOT NULL)
    ),
    marc_identifiers AS (
    ...
    records_actions AS (
      SELECT source_id, source_record_order, job_execution_id,
             (SELECT total_count FROM total_count_cte) AS total_count,
             (array_agg(entity_type) FILTER (WHERE entity_type IN (''MARC_BIBLIOGRAPHIC'', ''MARC_HOLDINGS'', ''MARC_AUTHORITY'')))[1] AS source_record_entity_type,
             MAX(tenant_id) FILTER (WHERE entity_type IN (''MARC_BIBLIOGRAPHIC'', ''MARC_HOLDINGS'', ''MARC_AUTHORITY'') AND tenant_id IS NOT NULL) AS source_record_tenant_id,
             (array_agg(error) FILTER (WHERE entity_type IN (''MARC_BIBLIOGRAPHIC'', ''MARC_HOLDINGS'', ''MARC_AUTHORITY'')))[1] AS source_entity_error
      FROM relevant_records
      GROUP BY source_id, source_record_order, job_execution_id
    ),
    rec_titles AS (
      SELECT DISTINCT source_id, title
      FROM relevant_records
      WHERE (entity_id IS NOT NULL OR action_status = ''ERROR'' or action_type = ''NON_MATCH'')
         OR (entity_type IN (''MARC_AUTHORITY'', ''MARC_BIBLIOGRAPHIC'', ''MARC_HOLDINGS'') AND title IS NOT NULL)
    ),
    rec_errors AS (
      SELECT source_id,
             CASE WHEN COUNT(*) = 1 THEN array_to_string(array_agg(error), '', '') ELSE ''['' || array_to_string(array_agg(error), '', '') || '']'' END AS error
      FROM relevant_records
      WHERE error != '''' GROUP BY source_id
    )
  SELECT records_actions.job_execution_id, records_actions.source_id AS incoming_record_id, mi.marc_entity_id::uuid AS source_id, records_actions.source_record_order, '''' as invoiceline_number,
         coalesce(rt.title, mh.title) AS title, coalesce(mb.action_type, ma.action_type, mh.action_type) AS source_record_action_status,
         records_actions.source_entity_error, records_actions.source_record_tenant_id, i.action_type AS instance_action_status,
         coalesce(i.entity_id, h.instance_id, it.instance_id) AS instance_entity_id, i.entity_hrid AS instance_entity_hrid,
         i.error AS instance_entity_error, i.tenant_id AS instance_entity_tenant_id, h.action_type AS holdings_action_status,
         coalesce(h.entity_id, it.holdings_id) AS holdings_entity_id, h.entity_hrid AS holdings_entity_hrid,
         h.permanent_location_id AS holdings_permanent_location_id, h.error AS holdings_entity_error, it.action_type AS item_action_status,
         it.entity_id AS item_entity_id, it.entity_hrid AS item_entity_hrid, it.error AS item_entity_error, it.holdings_id AS item_holdings_id,
         a.action_type AS authority_action_status, coalesce(a.entity_id, ma.entity_id) AS authority_entity_id,
         coalesce(a.error, ma.error) AS authority_entity_error, pl.action_type AS po_line_action_status, pl.entity_id AS po_line_entity_id,
         pl.entity_hrid AS po_line_entity_hrid, pl.error AS po_line_entity_error, pl.order_id AS order_entity_id,
         null AS invoice_action_status, null::text[] AS invoice_entity_id, null::text[] AS invoice_entity_hrid, null AS invoice_entity_error,
         null AS invoice_line_action_status, null AS invoice_line_entity_id, null AS invoice_line_entity_hrid, null AS invoice_line_entity_error,
         records_actions.total_count, null::UUID AS invoice_line_journal_record_id, records_actions.source_record_entity_type,
         ARRAY[records_actions.source_record_order] AS source_record_order_array, pl.action_type AS order_action_status, re.error
  FROM records_actions
         LEFT JOIN rec_titles rt ON rt.source_id = records_actions.source_id AND rt.title IS NOT NULL
         LEFT JOIN instances i ON i.source_id = records_actions.source_id
         LEFT JOIN holdings h ON h.source_id = records_actions.source_id
         LEFT JOIN items it ON it.source_id = records_actions.source_id
         LEFT JOIN po_lines pl ON pl.source_id = records_actions.source_id
         LEFT JOIN authorities a ON a.source_id = records_actions.source_id
         LEFT JOIN marc_authority ma ON ma.source_id = records_actions.source_id
         LEFT JOIN marc_bibliographic mb ON mb.source_id = records_actions.source_id
         LEFT JOIN marc_holdings mh ON mh.source_id = records_actions.source_id
         LEFT JOIN marc_identifiers mi ON mi.marc_source_id = records_actions.source_id
         LEFT JOIN rec_errors re ON re.source_id = records_actions.source_id
  ORDER BY %9$s %6$s;';
...
originalSQL TEXT := '
  ...
  FROM (
         SELECT journal_records.source_id, journal_records.source_record_order, journal_records.job_execution_id,
                array_agg(action_type ORDER BY array_position(array[''MATCH'', ''NON_MATCH'', ''UPDATE'', ''CREATE''], action_type)) FILTER (WHERE entity_type IN (''MARC_BIBLIOGRAPHIC'', ''MARC_HOLDINGS'', ''MARC_AUTHORITY'')) AS marc_actions,
                ...
                array_agg(action_type ORDER BY array_position(array[''CREATE'', ''UPDATE'', ''NON_MATCH'', ''MATCH''], action_type)) FILTER (WHERE entity_type = ''INSTANCE'' AND (entity_id IS NOT NULL OR action_type = ''NON_MATCH'')) AS instance_actions,
                count(journal_records.source_id) FILTER (WHERE entity_type = ''INSTANCE'' AND journal_records.error != '''') AS instance_errors_number,
                array_agg(action_type ORDER BY array_position(array[''CREATE'', ''UPDATE'', ''NON_MATCH''], action_type)) FILTER (WHERE entity_type = ''HOLDINGS'') AS holdings_actions,
                count(journal_records.source_id) FILTER (WHERE entity_type = ''HOLDINGS'' AND journal_records.error != '''') AS holdings_errors_number,
                array_agg(action_type ORDER BY array_position(array[''CREATE'', ''UPDATE'', ''NON_MATCH''], action_type)) FILTER (WHERE entity_type = ''ITEM'') AS item_actions,
                count(journal_records.source_id) FILTER (WHERE entity_type = ''ITEM'' AND journal_records.error != '''') AS item_errors_number,
                array_agg(action_type ORDER BY array_position(array[''CREATE'', ''UPDATE'', ''NON_MATCH''], action_type)) FILTER (WHERE entity_type = ''AUTHORITY'') AS authority_actions,
                count(journal_records.source_id) FILTER (WHERE entity_type = ''AUTHORITY'' AND journal_records.error != '''') AS authority_errors_number,
                array_agg(action_type ORDER BY array_position(array[''CREATE'', ''UPDATE'', ''NON_MATCH''], action_type)) FILTER (WHERE entity_type = ''PO_LINE'') AS po_line_actions,
...
BEGIN
  ...
  END IF;
    ...
    WHEN 'title' THEN
            ...
            v_orderByFinal := 'COALESCE(rt.title, mh.title)';

    WHEN 'error' THEN
            v_sortingField := 'error';
            v_orderByPagination := 'sort_error';
            v_orderByFinal := 're.error';
