...
public class ChunkProcessingServiceImpl extends AbstractChunkProcessingService {
  ...
  protected Future<Boolean> processRawRecordsChunk(RawRecordsDto incomingChunk, JobExecutionSourceChunk sourceChunk, String jobExecutionId, OkapiConnectionParams params) {
    ...
    updateJobExecutionProgress(jobExecutionId, incomingChunk, params)
      ...
      .onComplete(chunkProcessAr -> updateJobExecutionStatusIfAllChunksProcessed(jobExecutionId, params)
        .onComplete(jobUpdateAr -> promise.handle(chunkProcessAr.map(true))));
  ...
  public Future<Boolean> sendEventsWithStoredRecords(List<Record> createdRecords, String jobExecutionId, OkapiConnectionParams params) {
    return Future.failedFuture("Method is not supported");
  }
}
