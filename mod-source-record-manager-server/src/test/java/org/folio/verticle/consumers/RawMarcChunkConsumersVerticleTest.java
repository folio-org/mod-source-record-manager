...
public class RawMarcChunkConsumersVerticleTest extends AbstractRestTest {
  ...
  public void shouldFillInInstanceIdAndInstanceHridWhenRecordContains999FieldWithInstanceId() throws InterruptedException, IOException {
    ...
    kafkaRecord.addHeader(OKAPI_URL_HEADER, "http://localhost:" + snapshotMockServer.port(), UTF_8);
    ...
    Assert.assertEquals(1, recordCollection.getRecords().size());
    Record record = recordCollection.getRecords().get(0);
    Assert.assertNotNull(record.getExternalIdsHolder());

    Assert.assertEquals("e27a5374-0857-462e-ac84-fb4795229c7a", record.getExternalIdsHolder().getInstanceId());
    Assert.assertEquals("1007048", record.getExternalIdsHolder().getInstanceHrid());
  }
}
