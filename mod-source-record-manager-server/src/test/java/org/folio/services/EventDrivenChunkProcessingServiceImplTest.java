...
public class EventDrivenChunkProcessingServiceImplTest extends AbstractRestTest {
  ...
  public void shouldProcessChunkOfRawRecords(TestContext context) {
    ...
    future.setHandler(ar -> {
      ...
      EventContext eventContext = Json.decodeValue(event.getEventPayload(), EventContext.class);
      context.assertNotNull(eventContext.getProfileSnapshot());
      context.assertNotNull(eventContext.getCurrentNode());
      context.assertEquals(CREATED_SRS_MARC_BIB_RECORD.value(), eventContext.getEventType());
      context.assertNotNull(eventContext.getObjects().get(MARC_BIBLIOGRAPHIC.value()));
      async.complete();
    });
  }
